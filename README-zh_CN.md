...
[English](https://github.tools.sap/sfsfmcp/sap-jira-mcp/blob/main/README.md) | 中文
...
- 这个mcp从v2开始既支持由sap-auth-mcp获取的cookie调用jira，也支持基于api token的jira访问，请参考环境变量设置部分
- 注意，当选择使用coookie用于访问jira时，这个mcp需要配合[sap-auth-mcp](https://github.tools.sap/sfsfmcp/sap-auth-mcp)使用
- 注意，api token方式并非推荐方式，且申请api token有可能被拒绝
...
```
如果能完成运行并在终端得到输出版本 >= 1.1.0，就可以添加到你的支持MCP的客户端:
...
```
至此，sap-jira-mcp已经可以在你的客户端正常运行了，请注意，这个mcp需要依赖[sap-auth-mcp](https://github.tools.sap/sfsfmcp/sap-auth-mcp) 实现无缝认证。
请注意：AUTH_COOKIE_DIR这个路径是一个目录用于`sap-auth-mcp`来存放认证后得到的jira的cookie内容的文件`sap-cookie.json`，只需要保证这个路径对于mcp有读写权限即可，请加上jira特有的路径，因为所有共享使用`sap-auth-mcp`的mcp都会生成同名的cookie文件，为了避免覆盖，需要在路径上唯一。
...
Cookie 认证使用由 [sap-auth-mcp](https://github.tools.sap/sfsfmcp/sap-auth-mcp) 管理的 SSO 会话 cookies。
...
**注意：** Cookie 认证需要配合 [sap-auth-mcp](https://github.tools.sap/sfsfmcp/sap-auth-mcp) 来处理认证流程。
...
- `JIRA_API_TOKEN`：Jira API token（从 SAP Jira 管团队申请获取）
...
**"需要认证" 错误**
- 确保 [sap-auth-mcp](https://github.tools.sap/sfsfmcp/sap-auth-mcp) 已配置并认证
...
- **纯 HTTP 客户端**：基于 Cookie 的快速请求用于搜索操作
- **Cookie 存储**：自动错误驱动刷新的持久会话管理
- **HTML 内容提取**：从工单页面全面提取内容
- **与 sap-auth-mcp 集成**：无缝认证处理

## 认证流程

1. **加载 Cookie**：尝试从存储加载现有 cookies
2. **执行请求**：使用可用 cookies 进行 HTTP 请求
3. **错误驱动刷新**：认证错误触发 sap-auth-mcp 刷新 cookie
4. **会话持久化**：保存有效 cookies 供将来使用
...
用作备用的静态 JQL 示例存储在 `src/jql_examples.md` 中。这个 AI 友好的 markdown 文件包含：

- **JQL 示例**：带有描述的详细示例
- **备用元数据**：默认项目、状态、优先级和问题类型

要添加或修改示例：
1. 以标准 markdown 格式编辑 `src/jql_examples.md`
2. 运行 `npm run build` 将更新的文件复制到 `dist/`
3. 当动态元数据获取失败时，示例将作为 markdown 内容包含在内

Markdown 内容直接返回给 AI 客户端进行解析，使其易于维护和理解。
