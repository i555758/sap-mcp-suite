...
export class FormatterService {
   ...
   */
  formatRepositoryList(repositories: GitHubRepository[]): string {
  ...
  formatRepository(repo: GitHubRepository): string {
    ...
    output += `- Updated: ${new Date(repo.updated_at).toLocaleDateString()}\n`;
    output += `- Last Push: ${new Date(repo.pushed_at).toLocaleDateString()}\n\n`;
   ...
   * Format user details
   */
  formatUser(user: GitHubUser): string {
    ...
    if (user.created_at) {
      output += `- Joined: ${new Date(user.created_at).toLocaleDateString()}\n`;
    }
   ...
   * Format issue list
   */
  formatIssueList(issues: GitHubIssue[]): string {
    if (issues.length === 0) {
    ...
    issues.forEach(issue => {
      ...
      output += `  Created: ${new Date(issue.created_at).toLocaleDateString()}\n`;
  ...
  formatIssue(issue: GitHubIssue): string {
    ...
    if (issue.closed_at) {
      output += `**Closed:** ${new Date(issue.closed_at).toLocaleDateString()}\n`;
    }
   ...
   */
  formatPullRequestList(prs: GitHubPullRequest[]): string {
    if (prs.length === 0) {
  ...
  formatPullRequest(pr: GitHubPullRequest): string {
    ...
    if (pr.closed_at) {
      output += `**Closed:** ${new Date(pr.closed_at).toLocaleDateString()}\n`;
    }
    
    if (pr.merged_at) {
      output += `**Merged:** ${new Date(pr.merged_at).toLocaleDateString()}\n`;
      if (pr.merged_by) {
   ...
   */
  formatPullRequestReviews(reviews: any[]): string {
    if (reviews.length === 0) {
    ...
    reviews.forEach(review => {
      ...
      output += `  Submitted: ${new Date(review.submitted_at).toLocaleDateString()}\n`;
      if (review.body) {
   ...
   */
  formatPullRequestReviewComments(comments: any[]): string {
    ...
    comments.forEach(comment => {
      ...
      output += `  Created: ${new Date(comment.created_at).toLocaleDateString()}\n`;
      if (comment.updated_at !== comment.created_at) {
        output += `  Updated: ${new Date(comment.updated_at).toLocaleDateString()}\n`;
      }
   ...
   */
  formatPullRequestIssueComments(comments: any[]): string {
    if (comments.length === 0) {
   ...
   */
  formatPullRequestWithDetails(data: {
    pullRequest: GitHubPullRequest;
    reviews: any[];
    reviewComments: any[];
    issueComments: any[];
  }): string {
    let output = this.formatPullRequest(data.pullRequest);
    ...
    if (data.reviews.length > 0) {
      output += '## Reviews\n\n';
      output += this.formatPullRequestReviews(data.reviews);
    }
    
    if (data.reviewComments.length > 0) {
      output += '\n## Code Review Comments\n\n';
      output += this.formatPullRequestReviewComments(data.reviewComments);
    }
    
    if (data.issueComments.length > 0) {
      output += '\n## General Comments\n\n';
      output += this.formatPullRequestIssueComments(data.issueComments);
    }
   ...
   */
  formatPullRequestListWithDetails(data: {
   ...
   */
  formatComment(comment: any, commentType: 'issue' | 'review' = 'issue'): string {
   ...
   */
  formatCommentReply(reply: any, originalComment: any): string {
    let output = `# Reply Posted Successfully\n\n`;
    
    output += `**Reply ID:** ${reply.id}\n`;
    output += `**Author:** ${reply.user.login}\n`;
    output += `**Posted:** ${new Date(reply.created_at).toLocaleDateString()}\n`;
   ...
   */
  formatCommentUpdate(comment: any): string {
