...
/**
 * HTTP Client for Wiki - No Browser Dependencies
 * Supports both PAT authentication and cookie-based authentication via sap-auth
...
export class WikiHttpClient {
  ...
  /**
   * Check if an error is related to authentication failure
   */
  private isAuthRelatedError(error: unknown): boolean {
    if (error instanceof AuthRedirectError || error instanceof AuthError) {
      return true;
    }
  ...
  private async executeWithRetry<T>(
    ...
    return this.retryOnAuthFailure(requestFn, context);
  }
  ...
  private cleanHtmlContent(html: string): string {
    try {
      ...
      return `Error extracting content: ${extractErrorMessage(error)}`;
    }
  }
